...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const successRedirectUrl = `/donation/payment?order_id=${orderId}&status_code=200&transaction_status=capture`;
    ...
    window.snap.pay(snapToken, {
        ...
        onPending: function(result) {
            tampilkanModal();
            tampilkanStatus('warning', '⏳ Pembayaran sedang diproses. Segera selesaikan sesuai instruksi.');
            checkStatusAutomatically();
        },
        ...
        const interval = setInterval(() => {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    tampilkanStatus('success', '✅ Pembayaran terkonfirmasi! Mengalihkan...');
                    setTimeout(() => {
                        window.location.href = successRedirectUrl;
                    }, 1500);
                } else if (attempts >= maxAttempts) {
                    clearInterval(interval);
                    tampilkanStatus('warning', 'Konfirmasi otomatis memakan waktu. Silakan klik tombol Konfirmasi manual.');
        ...
        fetch(`/donation/status/${orderId}`)
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    tampilkanStatus('danger', '❌ Pembayaran belum terdeteksi. Silakan tunggu atau hubungi kami.');
                    this.disabled = false;
                    document.getElementById('konfirmasi-text').textContent = '✅ Ya, Konfirmasi!';
                    document.getElementById('konfirmasi-spinner').classList.add('d-none');
                }
            })
            .catch(() => {
